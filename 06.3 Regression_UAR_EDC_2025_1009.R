# ------------------------------------------------------------
# Purpose: Regression analysis of uric acid to albumin ratio (UAR) and individual EDCs
#
...
#-----------------------------------------------------------
run_models_cont_UAR <- function(data, chems, units, model_label, covars = c()) {
  for (i in seq_along(chems)) {
    chem <- chems[i]
    unit <- units[i]
    
    # Continuous model
    f_cont <- as.formula(paste("UAR ~", chem,
    ...
    results_UAR <<- bind_rows(results_UAR, est_cont)
  }
}
...
#-----------------------------------------------------------
run_models_cont_UAR(aim1_OR, pfases, pfas_unit, "Model3", 
                    covars = c("AGE","GENDER","racethnic","diabetes_new","education","BMI","Cotinine","PIR"))

run_models_cont_UAR(aim1_OR, phthals, phthal_unit, "Model3",
                    covars = c("AGE","GENDER","racethnic","diabetes_new","education","BMI","Cotinine","PIR","plogC"))

run_models_cont_UAR(aim1_OR, pahs, pah_unit, "Model3",
                    covars = c("AGE","GENDER","racethnic","diabetes_new","education","BMI","Cotinine","PIR","plogC"))
